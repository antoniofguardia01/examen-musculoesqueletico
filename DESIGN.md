...
On viewports wide enough to have real empty margins beside the 800px column (`min-width:1101px`), four small line-art specimens — a bone, a vertebra stack, a muscle-fiber wave, and a reticle echo of the hero mark — drift slowly (`bgDrift`, 9–15s, phase-staggered via negative `animation-delay` so they never move in unison) in the left/right gutters, `position:fixed` so they stay put through scroll. Treat these as one atmospheric layer/motif, not four separate effects: same keyframe family, same low-opacity budget (`.16`–`.2`), `pointer-events:none`, `aria-hidden`, gone entirely below 1101px (there is no empty space to hold them on tablet/mobile) and under `prefers-reduced-motion`.
...
- **Hero "plate" / landing**: a full-viewport (`min-height:88svh`, `92svh` on mobile) bordered frame that doubles as the page's landing screen — corner mono labels (subject / product line, stacked on mobile), a pulsing reticle mark in place of a logo, the display headline, a mono subline, a short lead sentence, a live mono stat line (`#heroStats`, computed from the real question/point counts at load — never hand-typed, so it can't go stale as content grows), a solid-accent CTA (`Iniciar práctica`), and a bouncing chevron scroll cue. No kicker/eyebrow above the headline (the headline carries its own weight). No institutional branding anywhere on the page (see PRODUCT.md Brand Commitments) — the CTA is a plain in-page anchor (`href="#practice"`, relies on the existing `scroll-behavior:smooth`) to the practice tool below; it never hides or gates that content, so the zero-friction principle holds even for a repeat visitor who reloads mid-session.
